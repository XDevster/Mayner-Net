<?php 
    ...
    if(isset($_POST['ban'])){
        if($_SESSION['user']['admin'] != 1){
            ...
            mysqli_query($db, "INSERT INTO banlist (user_id, reason) VALUES (" .$_SESSION['user']['user_id']. ", 'Был забанен за попытку взлома Mayner Net')");
            header("Location: $url");
        }
...
<form action="add.php?type=ban" method="post">
    ...
    if(isset($_POST['ban'])){
        if($_SESSION['user']['admin'] != 1){
            mysqli_query($db, "UPDATE users SET ban = 1 WHERE id = " .(int)$_SESSION['user']['user_id']);
            mysqli_query($db, "INSERT INTO banlist (user_id, reason) VALUES (" .$_SESSION['user']['user_id']. ", 'Был забанен за попытку взлома'");
            header("Location: $url");
        }
