...
<body>
		...
		<div class="main_app">
			...
			<form action="../methods/makepost.php" method="post" class="posting" enctype="multipart/form-data">
				...
				<div class="detail">
					...
					<div id="menu" style="display: none;">
						...
						<input type="file" name="file" class="file" accept=".swf">
					</div>
				</div>
			...
			<?php $data = mysqli_query($db, 'SELECT * FROM Flash ORDER BY date DESC LIMIT 10 OFFSET ' .(int)$_GET['p'] * 10); ?>
			
			<?php while($list = mysqli_fetch_assoc($data)): ?>

				<div class="block" id="Flash<?php echo($list['id']); ?>">
					...
					<?php 
						if($list['swf'] != NULL){
							echo('<img class="img" src="' .$list['.swf']. '">');
						}
					?>
