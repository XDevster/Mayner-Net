<?php
	...
	$banned = mysqli_fetch_assoc(mysqli_query($db, 'SELECT * FROM users WHERE ban = 1' .(int)$_GET['ban']));
    $anonimed = mysqli_fetch_assoc(mysqli_query($db, 'SELECT * FROM users WHERE anonim = 1' .(int)$_GET['anonim']));
?>
		...
		<div class="main">
			<?php if((int)$_GET['id'] == $_SESSION['user']['user_id']): ?>
				<div class="changeuser">
                    ...
                    | 
                    <a href="Emotions.php"><?php echo('Эмодзи'); ?></a>
					 | 
					<a href="yvidomlenia.php"><?php echo('Уведомления'); ?></a>
                    | 
					...
					<td class="info">
						<h1>
							<?php 
                                ...
                                if($data['anonim'] >= 1){
									    header("Location: /web/anonim.php ");
								}
							?>
						</h1>
					</td>
